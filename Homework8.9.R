...
#H1: there are sig effects of Hab type on totals.
lm1 <- lm(Total~Habitat, data = RedKnot)
summary(lm1)
#So, showing no significant effect of habitat on totals?
...
pp <- with(RedKnot,
           expand.grid(Year=levels(Year),habitat=levels(Habitat)))

pp2 <- pp
pp2$RedKnot <- predict(lmRK2,newdata=pp)

ggplot(pp2,aes(x=Year,y=Total,colour=Habitat))+geom_point()+

###ADD RAW data to plot###
###ALWAYS ADD RAW DATA!###
ggplot(pp2,aes(x=Year,y=Total,colour=Habitat))+ #pp2 is the dataframe with our predictions
  geom_point(size=4)+ #the large solid points show our predictions
  geom_point(data=RedKnot, aes(x=Year,y=Total,colour = Habitat), shape=1) #this shows our RAW data

# we specify open circles with "shape = 1"
plot(allEffects(lmRK2))
